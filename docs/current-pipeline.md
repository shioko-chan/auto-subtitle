...
flowchart TD
    ...
    L -->|确认歌唱| N[Qwen 歌声自适应 ASR<br/>目标 30 秒 / 20–38 秒]
...
确认歌唱区间先使用 Qwen3-ASR 的无时间戳模式作粗转写。离线窗口目标为 30 秒，允许
20–38 秒并保留 2 秒重叠；在目标点前后 5 秒内依次优先选择 Demucs 人声轨的非歌唱
gap、平滑 RMS 能量谷值和 phrase 边界，没有证据时才硬切，同时重新平衡不足 20 秒的
尾窗。各窗口通过文本重叠去重，歌唱请求使用空 context，避免在模糊歌声或间奏中复述
直播介绍。Qwen Forced Aligner 不处理歌唱；正式歌词命中后改由 pySHIRO 在持久化的
...
匹配后使用 Qwen 粗时间定位，在 Demucs 人声轨上切成不超过 20 秒的窗口，再由固定提交
的 pySHIRO 日文 v2 模型生成音素和歌词行时间。未匹配歌词的 call、ad-lib 和喊声被忽略；
句法上明确是普通讲话的片段会重新读取原始混音，用普通 Qwen + Forced Aligner 路径转写。

歌词定位采用半全局动态规划：完整使用按时间排列的歌唱 ASR，但允许免费跳过官方歌词的
开头和结尾，因此只唱副歌或歌曲中段时仍能定位。路径内部若跳过官方歌词行，则在前后可靠
锚点之间建立最多 20 秒的复查窗。复查使用空 context Qwen 重新识别 Demucs 人声轨，并
要求 ASR 与缺失歌词相似、人声活动率达标；随后比较仅含相邻锚点与加入缺失行两种 pySHIRO
对齐的每帧似然。只有扩展似然未明显恶化、绝对似然合格、音素时长合理且时间轴单调时才补回
缺失歌词。每次接受或拒绝及其证据均写入歌曲对齐审计。
