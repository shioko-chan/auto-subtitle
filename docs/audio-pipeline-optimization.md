...
- DiCoW: one isolated uv worker repairs only long-overlap local windows after Qwen
  releases its GPU allocation;
- MOSS-Transcribe-Diarize: retained as an optional comparison backend;
...
1. DiCoW and ERes2NetV2 workers batch all same-model job items into one
   invocation, so each model loads once per job.
2. Pass DiCoW and ERes2NetV2 input ranges through shared-memory descriptors.
3. Return ERes2NetV2 embeddings as JSON because they are small.
4. Keep DiCoW text/timestamps in JSON; no separated waveform is produced.
...
DiCoW consumes the shared 16 kHz waveform and returns text/timestamps. It produces no
separated waveform. Its validated local-window output is cached as JSON.
...
- Demucs depends on song candidates; DiCoW depends on true ordinary-timeline overlap;
...
- Demucs, DiCoW, and Qwen are GPU-heavy and should be mutually exclusive by
