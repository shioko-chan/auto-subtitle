...
class LocalSegmentationTests(unittest.TestCase):
    ...
    def test_local_merge_never_exceeds_source_width_guidance(self):
        cues = [
            Cue(0.0, 0.5, "あいうえお", "A"),
            Cue(0.5, 1.0, "かきくけこ", "A"),
            Cue(1.0, 1.5, "さしすせそ", "A"),
        ...
        tracks, _ = build_speaker_tracks(
            ...
            source_maximum_units=6.0,
        )

        self.assertEqual(
            [unit.text for unit in tracks[0].units], [cue.text for cue in cues]
        )
    ...
    def test_atomic_kinds_remain_single_units_and_audit_is_written(self):
        cues = [
            Cue(0, 1, "歌", "A", "singing"),
            Cue(1, 2, "重叠", "A", "conditioned_speech"),
        ]
        with tempfile.TemporaryDirectory() as temp:
            path = Path(temp) / "local-segmentation.json"
            tracks, _ = build_speaker_tracks(
                cues, SegmentationConfig(), analyzer=FakeAnalyzer(), audit_path=path
            )
            payload = json.loads(path.read_text(encoding="utf-8"))
        self.assertEqual(
            [unit.kind for unit in tracks[0].units], ["singing", "conditioned_speech"]
        )
    ...
    def test_remaining_unknown_beyond_half_second_is_discarded_and_audited(self):
        ...
        self.assertEqual(payload["speaker_reattributions"][-1]["reason"], "discarded")
    ...
    def test_english_units_preserve_spaces_and_use_punctuation_boundary(self):
        ...
        self.assertEqual(
            [unit.text for unit in tracks[0].units], ["Hello.", "But wait"]
        )
    ...
    def test_mixed_analysis_uses_sudachi_only_for_japanese_spans(self):
        values = SudachiAnalyzer().analyze_source("今日は Ready set です", "mixed")
